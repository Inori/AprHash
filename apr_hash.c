...
APR_DECLARE(void) apr_hash_destroy(apr_hash_t* h)
{
    ...
    while (old)
    {
        apr_hash_entry_t* next = old->next;
        apr_pfree(pool, old);
        old = next;
    }

    apr_pfree(pool, h->array);
    apr_pfree(pool, h);
}
...
APR_DECLARE(void) apr_hash_insert(apr_hash_t* ht, const void* key, 
    apr_ssize_t klen, const void* val)
{
    apr_hash_set(ht, key, klen, val);
}

APR_DECLARE(void) apr_hash_erase(apr_hash_t* ht, const void* key, 
    apr_ssize_t klen)
{
